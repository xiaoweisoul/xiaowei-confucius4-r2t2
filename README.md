...
模型(config.json `support_languages` / 官方库 `qwen_asr` `SUPPORTED_LANGUAGES`)
声明支持 30 种语言:中文、英文、粤语及阿拉伯/德/法/西/葡/印尼/意/韩/俄/泰/越/日/
土耳其/印地/马来/荷兰/瑞典/丹麦/芬兰/波兰/捷克/菲律宾/波斯/希腊/罗马尼亚/匈牙利/
马其顿语。

官方口径(模型 README Supported Languages):**重点优化中文和英文**,其余语言
保留"可用的跨语种流式能力"。

服务端约定(`ws_server.py`):

- header 传 `language` 默认为 `"zhen"`(代码内转为 None = 不指定,模型自动检测)
- **受缺陷 #1 制约,必须显式传 `"language": "Chinese"` 之类才会稳定**,自动检测
  路径在噪声场景会丢整轮文本
- 语言名自动规范化(首字母大写),不在支持列表内会报错拒绝

我方环境**只实测过中文**(312 条评测 + 多段样本);英文/中英混说(官方优化过的
场景,funasr_nano 不具备)、粤语与其他语言均未验证,上线前需补实测。
